...
public class RevitPluginParser extends AbstractDOMParser{
	...
	public Path parseDocumentWithMainOutline() {
		...
		if(nl != null && nl.getLength() > 0) {
			...
			Room r = getRoom(el);
			if (r==null){
				Logger.LOGGER.debug("Uwaga!!Blad parsowania obrysu dla pokoju id:");
	...
	public void parseDocument(){
		...
		if(nl != null && nl.getLength() > 0) {
			for(int i = 0 ; i < nl.getLength();i++) {
				...
				Room r = getRoom(el);
				if (r==null){
					 Logger.LOGGER.debug("Uwaga!!Blad parsowania XML dla pokoju id:");
					 Logger.LOGGER.debug (Integer.parseInt(el.getAttribute("id")));
					 continue;
				}
				rooms.add(r);
		...
		if(nl != null && nl.getLength() > 0) {
			for(int i = 0 ; i < nl.getLength();i++) {
				...
				BoundingBox bbox =  getBoundingBox(el);
				 ...
				 if (bbox==null ){
					 Logger.LOGGER.debug("Blad parsowania XML dla drzwi id:"+id);
				 }else {
	...
	private String getPolygonPoints(Element ele ) {
		...
		for(int i=0; i<nl.getLength(); i++){
			...
			if (i>0)Logger.LOGGER.debug("Poly diff r"+(i-1)+"->"+i+": " +(rx-rpx)+", "+(ry-rpy));
			//Logger.LOGGER.debug("Poly diff"+(i-1)+"->"+i+": " +(x-px)+", "+(y-py));
			px=x;
			py=y;
			rpx=rx;
			rpy=ry;
